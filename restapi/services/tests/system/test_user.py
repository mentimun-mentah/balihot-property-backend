...
class UserTest(BaseTest):
    def test_00_validation_register(self):
        ...
        # check length username, password, confirm_password
        with self.app() as client:
            res = client.post('/register',json={'username':'a','password':'a','confirm_password':'a'})
            self.assertEqual(400,res.status_code)
    ...
    def test_07_check_attempt_to_resend_email_back(self):
        user = User.query.filter_by(email=self.EMAIL_TEST).first()
    ...
    def test_17_send_email_reset_password_not_activated(self):
        with self.app() as client:
    ...
    def test_18_send_email_reset_password_not_in_database(self):
        # set user activated
        user = User.query.filter_by(email=self.EMAIL_TEST).first()
        user.confirmation.activated = True
        user.confirmation.save_to_db()
    ...
    def test_19_email_reset_password_in_database(self):
    ...
    def test_20_validation_reset_password(self):
    ...
    def test_21_token_not_found_reset_password(self):
        with self.app() as client:
    ...
    def test_24_reset_password(self):
        password_reset = PasswordReset.query.filter_by(email=self.EMAIL_TEST).first()
        with self.app() as client:
    ...
    def test_25_validation_add_password(self):
        # login user
    ...
    def test_26_add_password_already_have_password(self):
    ...
    def test_27_add_password_to_user(self):
        user = User.query.filter_by(email=self.EMAIL_TEST).first()
        user.password = None
        user.save_to_db()
        # add new password
    ...
    def test_28_validation_update_password(self):
    ...
    def test_29_update_password_not_match_in_db(self):
        user = User.query.filter_by(email=self.EMAIL_TEST).first()
    ...
    def test_30_update_password_user(self):
        with self.app() as client:
    ...
    def test_31_validation_update_avatar_user(self):
        # avatar not found
        with self.app() as client:
    ...
    def test_32_update_avatar_user(self):
    ...
    def test_35_get_data_current_user(self):
        with self.app() as client:
