...
class GetUpdateDeleteType(Resource):
    ...
    @admin_required
    def get(self,id: int):
        type_db = Type.query.filter_by(id=id).first_or_404('Type not found')
        return _type_schema.dump(type_db), 200
    ...
    def put(self,id: int):
        ...
        data = request.get_json()
        args = _type_schema.load(data)
        if type_db.name != args['name'] and Type.query.filter_by(name=args['name']).first():
            raise ValidationError({'name':['The name has already been taken.']})

        type_db.name = args['name']
        type_db.save_to_db()
